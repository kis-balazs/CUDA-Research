...
"Atomic" we are referring to the indivisibility concept, where a thing cannot be broken down further.
...
Since there is a work limit done on a single piece of memory per unit time throughout an atomic operation, speed loss will happen. It is hardware guaranteed to be memory safe at a cost of speed.
...
Atomics are a sort of very fast, hardware-level mutex operation. It's as if each atomic operation does the following steps:
...
- [Mutual Exclusion Explanation](https://www.youtube.com/watch?v=MqnpIwN7dz0&t)
- "Mutual":
